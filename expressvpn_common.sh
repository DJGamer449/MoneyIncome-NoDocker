...
slovenia latvia cyprus chile albania slovakia uzbekistan isle-of-man estonia colombia mexico kazakhstan malta georgia mongolia
algeria uruguay guatemala peru venezuela ecuador serbia north-macedonia bosnia-and-herzegovina uk-midlands uk-east-london uk-tottenham
uk-london uk-docklands uk-wembley india-(via-uk) india-(via-singapore) australia-melbourne australia-sydney-2 australia-brisbane
 australia-perth australia-woolloomooloo australia-sydney australia-adelaide italy-milan italy-cosenza italy-naples
netherlands-rotterdam netherlands-the-hague netherlands-amsterdam brazil-2 brazil philippines canada-toronto-2 canada-vancouver
canada-montreal canada-toronto macau cambodia kenya andorra armenia belarus monaco jersey montenegro bangladesh bhutan brunei laos
myanmar nepal pakistan sri-lanka panama sweden-2 sweden austria germany-nuremberg germany-frankfurt-1 germany-frankfurt-3
spain-barcelona spain-madrid spain-barcelona-2 japan-yokohama japan-tokyo japan-shibuya japan-osaka bolivia guam ghana
 dominican-republic jamaica puerto-rico bermuda trinidad-and-tobago cayman-islands cuba honduras lebanon morocco united-arab-emirates
azerbaijan portugal poland ireland finland lithuania czech-republic south-korea-2 denmark egypt belgium romania ukraine
...
prompt_vpn_settings() {
  ...
  if [[ -z "${VPN_PROTOCOL:-}" ]]; then
    read -rp "Protocol [default: auto]: " VPN_PROTOCOL
    VPN_PROTOCOL="${VPN_PROTOCOL:-auto}"
  fi
  [[ "$INSTANCE_COUNT" =~ ^[0-9]+$ ]] && ((INSTANCE_COUNT>0)) || { echo "Invalid instance count"; exit 1; }
  SELECTED_REGIONS=()
  for ((i=1;i<=INSTANCE_COUNT;i++)); do
    dflt="${REGIONS[$(((i-1)%${#REGIONS[@]}))]}"
    read -rp "Region for instance $i [default: $dflt]: " reg
    SELECTED_REGIONS+=("${reg:-$dflt}")
  done
}

ensure_expressvpn_binaries() {
  local base_dir="$(cd "$(dirname "$0")" && pwd)"
  EXPRESSVPN_DAEMON="$base_dir/app/expressvpn/bin/expressvpn-daemon"
  EXPRESSVPNCTL="$base_dir/app/expressvpn/bin/expressvpnctl"
  [[ -x "$EXPRESSVPN_DAEMON" ]] || { echo "Missing $EXPRESSVPN_DAEMON"; exit 1; }
  [[ -x "$EXPRESSVPNCTL" ]] || { echo "Missing $EXPRESSVPNCTL"; exit 1; }
}

start_expressvpn_in_ns() {
  local ns="$1" idx="$2" region="$3" workdir="$4"
  local root="$workdir/expressvpn_$idx"
  mkdir -p "$root/home" "$root/config" "$root/data" "$root/runtime" "$root/log"
  ip netns exec "$ns" groupadd -f expressvpn >/dev/null 2>&1 || true
  ip netns exec "$ns" env HOME="$root/home" XDG_CONFIG_HOME="$root/config" XDG_DATA_HOME="$root/data" XDG_RUNTIME_DIR="$root/runtime" bash -lc "
    '$EXPRESSVPN_DAEMON' > '$root/log/daemon.log' 2>&1 &
    sleep 2
