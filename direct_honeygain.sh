...
require_root() {
  ...
  [[ -f "$HONEYGAIN_ACCOUNTS_FILE" ]] || { echo "Honeygain accounts file not found: $HONEYGAIN_ACCOUNTS_FILE"; exit 1; }
}
...
load_accounts() {
  ...
  (( ${#HONEYGAIN_ACCOUNTS[@]} > 0 )) || { echo "No Honeygain accounts in $HONEYGAIN_ACCOUNTS_FILE"; exit 1; }
}
...
start_app_instance() {
  ...
  local account="${HONEYGAIN_ACCOUNTS[$(( (idx-1) % ${#HONEYGAIN_ACCOUNTS[@]} ))]}"
  local email="${account%%|*}"
  local password="${account#*|}"
  local raw_uuid device_name
  raw_uuid="$(uuidgen | tr -d '-')"
  device_name="hg-${raw_uuid:0:12}"

  echo "[$idx] Starting Honeygain for $email in netns=$ns"
  ip netns exec "$ns" bash -lc "cd '$(pwd)'; export HOME='$inst_dir'; '$HONEYGAIN_BIN' -tou-accept -email '$email' -pass '$password' -device '$device_name'" >"$WORKDIR/app_${idx}.log" 2>&1 &
...
main() {
  ...
  echo "Started $INSTANCE_COUNT Honeygain instance(s) with per-namespace ExpressVPN."
  echo "Logs: $WORKDIR"
  wait
}
