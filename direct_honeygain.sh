...
require_root(){ [[ $EUID -eq 0 ]] || exit 1; [[ -x ./app/honeygain_file/honeygain ]] || { echo "Missing honeygain binary"; exit 1; }; }
...
trap cleanup EXIT
main(){ require_root; ensure_expressvpn_binaries; setup_nat_once; prompt_vpn_settings; IFS=$'\n' read -r -d '' -a ACCOUNTS < <(printf '%s\0' "${HONEYGAIN_ACCOUNTS:-}") || true; for i in $(seq 1 "$INSTANCE_COUNT"); do ns="$(create_ns "$i")"; start_expressvpn_in_ns "$ns" "$i" "${SELECTED_REGIONS[$((i-1))]}" "$WORKDIR"; account="${ACCOUNTS[$(( (i-1) % ${#ACCOUNTS[@]:-1} ))]:-}"; email="${account%%|*}"; pass="${account#*|}"; ip netns exec "$ns" bash -lc "cd '$(pwd)/app/honeygain_file'; LD_LIBRARY_PATH=. ./honeygain -tou-accept -email '$email' -pass '$pass'" >"$WORKDIR/app_${i}.log" 2>&1 & echo $! >"$WORKDIR/app_${i}.pid"; done; wait; }
main "$@"
