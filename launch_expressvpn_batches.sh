...
IMAGE="${IMAGE:-misioslav/expressvpn}"
...
wait_for_connected_batch() {
  ...
  while ((${#pending[@]} > 0)); do
    ...
    for name in "${pending[@]}"; do
      state="$(docker exec "${name}" bash -lc "${CTL_IN_CONTAINER} get connectionstate 2>/dev/null || true" 2>/dev/null || true)"
    ...
    if ((${#pending[@]} > 0)); then
      echo "Waiting for ${#pending[@]} container(s) in current batch..."
      sleep 3
    fi
  done
}
...
start_batch() {
  ...
  for i in $(seq "${start_idx}" "${end_idx}"); do
    ...
    docker run -d \
      ...
      --name="${name}" \
      "${IMAGE}" \
      bash "${START_SCRIPT_IN_CONTAINER}" >/dev/null

    batch_names+=("${name}")
    sleep 0.3
  done

  echo "Waiting for batch to connect (${#batch_names[@]} instances)..."
...
echo "All ${total} containers launched and connected."
