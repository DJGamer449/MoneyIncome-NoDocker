...
EXPRESSVPN_ROOT="${EXPRESSVPN_ROOT:-/opt/expressvpn/earnapp_expressvpn}"
...
start_instance() {
  ...
  ip netns exec "$ns" bash -lc '/usr/bin/earnapp start && sleep 5 && exec /usr/bin/earnapp run' >"$WORKDIR/app_${idx}.log" 2>&1 &
