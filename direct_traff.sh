...
prompt_inputs(){ [[ -n "${TRAFF_TOKEN:-}" ]] || read -rp "Enter Traff token: " TRAFF_TOKEN; [[ -n "${EXPRESSVPN_KEY:-}" ]] || { read -rsp "Enter ExpressVPN activation key/token: " EXPRESSVPN_KEY; echo; }; [[ -n "${INSTANCE_COUNT:-}" ]] || read -rp "How many Traff instances to run? " INSTANCE_COUNT; [[ "$INSTANCE_COUNT" =~ ^[0-9]+$ ]] || exit 1; ((INSTANCE_COUNT>0)) || exit 1; APP_CMD=( ./app/cli start accept --token "$TRAFF_TOKEN" ); }
...
start_app(){ local ns="$1" idx="$2" inst="$WORKDIR/inst_${idx}"; mkdir -p "$inst"; ip netns exec "$ns" bash -lc "cd '$SCRIPT_DIR'; export HOME='$inst'; ${APP_CMD[*]}" >"$WORKDIR/app_${idx}.log" 2>&1 & echo $! > "$WORKDIR/app_${idx}.pid"; }
