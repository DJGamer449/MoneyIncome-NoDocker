...
start_app(){ local ns="$1" idx="$2" inst="$WORKDIR/inst_${idx}"; mkdir -p "$inst"; ip netns exec "$ns" bash -lc "cd '$(pwd)'; export HOME='$inst'; ${APP_CMD[*]}" >"$WORKDIR/app_${idx}.log" 2>&1 & echo $! > "$WORKDIR/app_${idx}.pid"; }
