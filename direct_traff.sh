...
main(){ require_root; ensure_expressvpn_binaries; setup_nat_once; prompt_vpn_settings; for i in $(seq 1 "$INSTANCE_COUNT"); do ns="$(create_ns_with_veth "$i")"; start_instance "$i" "$ns" "${SELECTED_REGIONS[$((i-1))]}"; done; echo "Started $INSTANCE_COUNT traff instance(s) via ExpressVPN"; wait; }
main "$@"
