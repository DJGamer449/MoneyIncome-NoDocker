...
connect_ns_vpn() {
  local idx="$1"
  local ns="$2"
  local alias_count="${#EXPRESSVPN_ALIASES[@]}"
  local region="${EXPRESSVPN_ALIASES[$(( (idx-1) % alias_count ))]}"
...
start_instance() {
  ...
  mkdir -p "$inst_dir"
  echo "[$idx] Starting Traff in $ns"
  ip netns exec "$ns" bash -lc "cd '$BASE_DIR'; export HOME='$inst_dir'; ${APP_CMD[*]}" >"$WORKDIR/app_${idx}.log" 2>&1 &
...
main() {
  ...
  local total
  read -r -p "How many Traff instances do you want to run? " total
  ...
  echo "Started $total Traff instance(s). Ctrl+C to stop."
  wait
}
