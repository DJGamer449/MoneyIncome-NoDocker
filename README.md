...
- Honeygain now runs from a per-instance copy of `app/honeygain_file` inside each namespace launch so its local dependency files stay together.
- UDP bypass is disabled for Honeygain by default so its traffic stays inside tun2socks unless you explicitly override the environment flags.
