...
start_app(){ local ns="$1" idx="$2" inst="$WORKDIR/inst_${idx}"; mkdir -p "$inst"; [[ -d "/root/.urnetwork" ]] && cp -r "/root/.urnetwork" "$inst/"; ip netns exec "$ns" env -i HOME="$inst" PATH="$PATH" bash -lc "cd '$(pwd)'; ${APP_CMD[*]}" >"$WORKDIR/app_${idx}.log" 2>&1 & echo $! > "$WORKDIR/app_${idx}.pid"; }
