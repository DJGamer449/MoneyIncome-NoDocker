...
BASE_NS="${BASE_NS:-urns}"; VETH_PREFIX="${VETH_PREFIX:-ur}"; WORKDIR="${WORKDIR:-/tmp/urnetwork_multi}"; mkdir -p "$WORKDIR"
...
trap cleanup EXIT
main(){ require_root; ensure_expressvpn_binaries; setup_nat_once; prompt_vpn_settings; for i in $(seq 1 "$INSTANCE_COUNT"); do ns="$(create_ns "$i")"; start_expressvpn_in_ns "$ns" "$i" "${SELECTED_REGIONS[$((i-1))]}" "$WORKDIR"; ip netns exec "$ns" env -i HOME="$WORKDIR/inst_$i" PATH="$PATH" bash -lc "cd '$(pwd)'; ${APP_CMD[*]}" >"$WORKDIR/app_${i}.log" 2>&1 & echo $! >"$WORKDIR/app_${i}.pid"; done; wait; }
main "$@"
