...
INSTALL_SCRIPT="$BASE_DIR/app/expressvpn/start.sh"
PACKETSTREAM_SCRIPT="$BASE_DIR/direct_packetstream.sh"
...
run_earnapp() {
  ...
  sudo BASE_NS=earnns VETH_PREFIX=earn WORKDIR=/tmp/earnapp_multi bash "$EARNAPP_SCRIPT" &
  PIDS+=($!)
}
...
run_traff() {
  ...
  echo "Starting Traff..."
  sudo BASE_NS=traffns VETH_PREFIX=traff WORKDIR=/tmp/traff_multi TRAFF_TOKEN="$TRAFF_TOKEN" bash "$TRAFF_SCRIPT" &
  PIDS+=($!)
}
...
run_packetstream() {
  ...
  sudo BASE_NS=psns VETH_PREFIX=ps WORKDIR=/tmp/ps_multi PS_TOKEN="$PS_TOKEN" bash "$PACKETSTREAM_SCRIPT" &
  PIDS+=($!)
}
...
run_castar() {
  ...
  sudo BASE_NS=castarns VETH_PREFIX=castar WORKDIR=/tmp/castar_multi CASTAR_KEY="$CASTAR_KEY" bash "$CASTAR_SCRIPT" &
  PIDS+=($!)
}
...
run_urnetwork() {
  ...
  sudo BASE_NS=urns VETH_PREFIX=ur WORKDIR=/tmp/ur_multi bash "$UR_SCRIPT" &
  PIDS+=($!)
}
...
menu() {
  ...
  echo "5) Run Castar"
  echo "6) Run ExpressVPN start script (manual)"
...
while true; do
  ...
  case "$opt" in
    ...
    5) run_castar ; wait ;;
    6) echo "Use direct_* scripts (they now start ExpressVPN per namespace)." ; wait ;;
