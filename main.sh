...
PS_SCRIPT="$BASE_DIR/direct_packetstream.sh"
...
run_earnapp() {
  echo "Starting EarnApp with ExpressVPN..."
  sudo BASE_NS=earnns WORKDIR=/tmp/earnapp_multi bash "$EARNAPP_SCRIPT" &
  PIDS+=($!)
}

run_traff() {
  if [[ -z "$TRAFF_TOKEN" ]]; then echo "Traff token not set."; return; fi
  echo "Starting Traff with ExpressVPN..."
  sudo BASE_NS=traffns WORKDIR=/tmp/traff_multi TRAFF_TOKEN="$TRAFF_TOKEN" bash "$TRAFF_SCRIPT" &
  PIDS+=($!)
}

run_packetstream() {
  if [[ -z "$PS_TOKEN" ]]; then echo "PacketStream token not set."; return; fi
  echo "Starting PacketStream with ExpressVPN..."
  sudo BASE_NS=psns WORKDIR=/tmp/ps_multi PS_TOKEN="$PS_TOKEN" bash "$PS_SCRIPT" &
  PIDS+=($!)
}

run_castar() {
  if [[ -z "$CASTAR_KEY" ]]; then echo "Castar key not set."; return; fi
  echo "Starting Castar with ExpressVPN..."
  sudo BASE_NS=castarns WORKDIR=/tmp/castar_multi CASTAR_KEY="$CASTAR_KEY" bash "$BASE_DIR/direct_castar.sh" &
  PIDS+=($!)
}

run_urnetwork() {
  echo "Starting UrNetwork with ExpressVPN..."
  if [[ ! -f "$HOME/.urnetwork/jwt" ]]; then
    ./app/provider auth
  fi
  sudo BASE_NS=urns WORKDIR=/tmp/ur_multi bash "$UR_SCRIPT" &
  PIDS+=($!)
}
...
run_wipter() {
  ...
  echo "Starting Wipter with ExpressVPN..."
  sudo BASE_NS=wipterns WORKDIR=/tmp/wipter_multi WIPTER_EMAIL="$WIPTER_EMAIL" WIPTER_PASSWORD="$WIPTER_PASSWORD" bash "$WIPTER_SCRIPT" &
  PIDS+=($!)
}
...
run_honeygain() {
  ...
  local first="${HONEYGAIN_ACCOUNTS[0]}"
  local email="${first%%|*}"
  local password="${first#*|}"
  echo "Starting Honeygain with ExpressVPN..."
  sudo BASE_NS=honeyns WORKDIR=/tmp/honeygain_multi HG_EMAIL="$email" HG_PASSWORD="$password" bash "$HONEYGAIN_SCRIPT" &
  PIDS+=($!)
}
