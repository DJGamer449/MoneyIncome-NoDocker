#!/usr/bin/env bash
set -euo pipefail
APP_CMD=( ./app/cli start accept --token "key-replace" )
BASE_NS="${BASE_NS:-castarns}"; VETH_PREFIX="${VETH_PREFIX:-castar}"; WORKDIR="${WORKDIR:-/tmp/castar_multi}"; mkdir -p "$WORKDIR"
